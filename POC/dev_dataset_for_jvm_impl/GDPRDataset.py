...
class GDPRDataset():
    ...
    def __init__(self, dataset_dir):
        ...
        '''
        df['content'] = None
        df['page_source_html'] = None
        df['content_removed'] = None
        df['page_source_cleaned_html'] = None
        df['page_source_removed_html'] = None
        '''

        # Row by row read in the values from the files
        for i in range(len(df)):
            '''
            def try_open_and_read(path, dataframe, column_name):
                try:
                    with open(path) as f:
                        dataframe.loc[i][column_name] = f.read()
                except:
                    pass

            try_open_and_read(self.dataset_dir + f"/{i}/content.txt", df, 'content')

            try_open_and_read(self.dataset_dir + f"/{i}/page_source.html", df, 'page_source_html')

            try_open_and_read(self.dataset_dir + f"/{i}/content_removed.txt", df, 'content_removed')

            try_open_and_read(self.dataset_dir + f"/{i}/page_source_cleaned.html", df, 'page_source_cleaned_html')

            try_open_and_read(self.dataset_dir + f"/{i}/page_source_removed.html", df, 'page_source_removed_html')
            '''
